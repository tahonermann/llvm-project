...
void Sema::handleLambdaNumbering(
  ...
  if (!MCtx && (getLangOpts().CUDA || getLangOpts().SYCLIsDevice ||
    ...
    // Force lambda numbering in CUDA/HIP/SYCL as we need to name lambdas
    // following ODR. Both device- and host-compilation need to have a
    // consistent naming on kernel functions. As lambdas are potential part of
    // these `__global__` function names, they needs numbering following ODR.
